...
<div class="card">
  <div class="card-header bg-light">
    <div class="row align-items-center">
        <div class="col">
            <h5 class="mb-0" id="followers">Loanees
              <span class="d-none d-sm-inline-block">({{ $borrowers->count() }})</span>
                  </h5>
        </div>
            ...
            <a class="font-sans-serif" href="#!" data-bs-toggle="modal" data-bs-target="#addproductcat">
                <button class="btn btn-falcon-default btn-sm" type="button" data-bs-toggle="modal" data-bs-target="#addagent" >
                    <svg class="svg-inline--fa fa-plus fa-w-14" data-fa-transform="shrink-3 down-2" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="plus" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" data-fa-i2svg="" style="transform-origin: 0.4375em 0.625em;"><g transform="translate(224 256)"><g transform="translate(0, 64)  scale(0.8125, 0.8125)  rotate(0 0 0)"><path fill="currentColor" d="M416 208H272V64c0-17.67-14.33-32-32-32h-32c-17.67 0-32 14.33-32 32v144H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h144v144c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32V304h144c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" transform="translate(-224 -256)"></path></g></g></svg><!-- <span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span> Font Awesome fontawesome.com -->
                        <span class="d-none d-sm-inline-block ms-1">Add Loanee</span>
                </button>
        ...
        <table class="table data-table table-bordered table-striped fs--1 mb-0">
          <thead class="bg-200 text-900">
            <tr>
              ...
              <th class="sort pe-1 align-middle white-space-nowrap" data-sort="name">Loanee's Name</th>
              ...
              <th class="sort pe-1 align-middle white-space-nowrap text-center" data-sort="name">Loanee ID</th>
              <th class="sort pe-1 align-middle white-space-nowrap text-center" data-sort="name">Branch</th>
          ...
          <tbody class="list">
            @foreach($borrowers as $key => $agent)
              <tr class="btn-reveal-trigger">
                <td class="sn">{{ ++$key }}</td>
                <td class="name align-middle white-space-nowrap py-2">
                  <a href="{{ route('agent.view-borrower', $agent->id) }}">
              ...
              <td class="phone align-middle white-space-nowrap py-2 text-center">
                ...
                <td class="name align-middle white-space-nowrap py-2 text-center">
                  ...
                  @else
                    <small class="badge fw-semi-bold rounded-pill badge-subtle-primary">BLACKLISTED</small>
                  @endif
                </td>
                <td class="align-middle white-space-nowrap py-2 text-center">
                      ...
                      <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="customer-dropdown-0">
                          <div class="py-2"><a class="dropdown-item text-primary" href="{{ route('agent.view-borrower', $agent->id) }}">View</a></div>
              ...
              <div class="modal fade" id="deleteAgent{{ $agent->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 500px">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('borrower.destroy') }}" method="POST">
                            @csrf
                            @method('DELETE')
              ...
              <div class="modal fade" id="editAgent{{ $agent->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                    <form action="{{ route('admin.borrowerupdate', ['id' => $agent->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div>
                                  ...
                                  <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                          ...
                                          <input class="form-control @error('location') is-invalid @enderror" id="validationCustom03" value="{{ old('name', $agent->address) }}" name="location" type="text" autocomplete="on" placeholder="location"/>
                                          @error('location')
                                              <div class="invalid-feedback">{{ $message }}</div>
                                          @enderror
                                        </div>
                                      ...
                                      <div class="col-md-12">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" id="status" name="status">
                                                ...
                                                <option value="blacklisted" {{ old('status', $agent->status) === 'blacklisted' ? 'selected' : '' }}>BLACKLISTED</option>
                                            </select>
                                        </div>
...
<div class="modal fade" id="addagent" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <form action="{{ route('admin.storeborrower') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate="novalidate">
            @csrf
            <div class="modal-content position-relative">
                <div class="position-absolute top-0 end-0 mt-2 me-2 z-1">
                    <button class="btn-close btn btn-sm btn-circle d-flex flex-center transition-base"
                ...
                <div class="modal-body p-0">
                    <div class="rounded-top-3 py-3 ps-4 pe-6 bg-light">
                        <h4 class="mb-1" id="modalExampleDemoLabel">Add Loanee </h4>
                    </div>
                    <div class="p-4 pb-0">
                        <div class="row">
                            <div class="col-md-12">
                          ...
                          <div class="col-md-12">
                            <div class="mb-3">
                              <label for="validationCustom05" class="form-label">NIDA no.</label>
                              <input class="form-control @error('national_id') is-invalid @enderror" id="validationCustom05" value="{{ old('national_id') }}" type="text" name="national_id" autocomplete="on" placeholder="Loanee's national ID number" required />
                              @error('national_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                              @enderror
                            </div>
                          ...
                          <div class="col-md-12">
                            <div class="mb-3">
                              ...
                              <input class="form-control @error('address') is-invalid @enderror" id="validationCustom05" value="{{ old('address') }}" type="text" name="address" autocomplete="on" placeholder="Loanee's address" required />
                              @error('address')
                                <div class="invalid-feedback">{{ $message }}</div>
                              @enderror
                            </div>
                        ...
                        <div class="col-md-12">
                          <div class="mb-3">
                            <label for="validationCustom06" class="form-label">Employment Status</label>
                            <select class="form-select js-choice @error('employment_status') is-invalid @enderror" id="organizerSingle2" size="1" required="required" name="employment_status" aria-label="Default select example" data-options='{"removeItemButton":true,"placeholder":true}'>
                        ...
                        <div class="col-md-12">
                            <div class="mb-3">
                              <label for="validationCustom05" class="form-label">Employment Name</label>
                              <input class="form-control @error('employment_name') is-invalid @enderror" id="validationCustom05" value="{{ old('employment_name') }}" type="text" name="employment_name" autocomplete="on" placeholder="Loanee's employment name" required />
                        ...
                        <div class="col-md-12">
                            <div class="mb-3">
                                <label for="validationCustomIncome" class="form-label">Income</label>
                                <input
                                    class="form-control @error('income') is-invalid @enderror"
                                    id="validationCustomIncome"
                                    value="{{ old('income', $model->income ?? '') }}"
                                    type="number"
                                    name="income"
                                    step="0.01"
                                    min="0"
